...
The reusable framework lives in the **repository root**; this folder only
holds the driver, the build script, the weights, and the input images:

```
DNN_FHE/                            (repo root)
├── include/                        public headers (consumed via #include "...")
│   ├── bench/
...
│
├── accuracy.cpp                    optional batch-accuracy driver (gated by -DBUILD_ACCURACY=ON)
│
└── mvb/                            *** this folder ***
    ├── CMakeLists.txt              build script: locates ../include, ../src, ../accuracy.cpp
...
The build is driven from `mvb/CMakeLists.txt`. It declares
`REPO_ROOT = ${CMAKE_CURRENT_SOURCE_DIR}/..` and compiles:

* `libfhednn.a` — the framework, sources globbed from `../src/{network,io,bench}/*.cpp`,
  public headers (including the vendored `stb_image.h`) exposed from `../include`.
* `main`       — driver living in this folder, links `fhednn`.
* `accuracy`   — optional batch harness sourced from `../accuracy.cpp`,
  built only when `-DBUILD_ACCURACY=ON`.

This lets multiple sub-projects (e.g. a hypothetical `mvb_chebyshev/`) share
the framework code without duplication: each sub-project just provides its
own `CMakeLists.txt` + `main.cpp` + data and points at the same `../src`,
...
FHEContext ctx;                    // defaults match the original DiNN100 setup
Network    net;
net.SetInputShift(1)               // pixels are {-1,+1}; shift to {0,2}
   .Linear(W1, b1)                 // 784 -> 100
...
[`../src/network/activation.cpp`](../src/network/activation.cpp) — each factory
is a one-shot lambda + metadata, ~10 lines.
