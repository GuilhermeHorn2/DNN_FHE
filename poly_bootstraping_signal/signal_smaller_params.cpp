...
FHEConfig setup_fhe_environment() {
    ...
    // Você pode tentar comentar a linha abaixo para ver se com a redução
    // de profundidade o OpenFHE consegue rodar com RingDim 2^16 automaticamente
    parameters.SetRingDim(1 << 17);

    // Scaling parameters reduzidos para economizar memória e acelerar
    ...
    // Margem reduzida de 10 para 7 devido à redução do grau de Chebyshev.
...
Ciphertext<DCRTPoly> apply_approx_activation(Ciphertext<DCRTPoly> ct_input, FHEConfig& config) {
    ...
    // Grau reduzido de 7 para 3 para poupar níveis
    uint32_t degree = 3; 
